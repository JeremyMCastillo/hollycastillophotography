...
class ModelTltBlogTltBlog extends Model {
	public function addTltBlog($data) {
		$this->event->trigger('pre.admin.tltblog.add', $data);

		$this->db->query("INSERT INTO " . DB_PREFIX . "tltblog SET sort_order = '" . (int)$data['sort_order'] . "', image = '" . (isset($data['image']) ? $this->db->escape($data['image']) : '') . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', show_title = '" . (isset($data['show_title']) ? (int)$data['show_title'] : 0) . "', status = '" . (int)$data['status'] . "', show_description = '" . (isset($data['show_description']) ? (int)$data['show_description'] : 0) . "', date_added=now()");
		...
		if (isset($data['keyword'])) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "tltblog_url_alias SET query = 'tltblog_id=" . (int)$tltblog_id . "', keyword = '" . $this->db->escape($data['keyword']) . "'");
		}

		$this->cache->delete('tltblog');

		$this->event->trigger('post.admin.tltblog.add', $tltblog_id);

		return $tltblog_id;
	}

	public function editTltBlog($tltblog_id, $data) {
		$this->event->trigger('pre.admin.tltblog.edit', $data);
		...
		$this->event->trigger('post.admin.tltblog.edit', $tltblog_id);
	}
	...
	public function deleteTltBlog($tltblog_id) {
		$this->event->trigger('pre.admin.tltblog.delete', $tltblog_id);
		...
		$this->event->trigger('post.admin.tltblog.delete', $tltblog_id);
	}

	public function getTltBlog($tltblog_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT keyword FROM " . DB_PREFIX . "tltblog_url_alias WHERE query = 'tltblog_id=" . (int)$tltblog_id . "') AS keyword FROM " . DB_PREFIX . "tltblog WHERE tltblog_id = '" . (int)$tltblog_id . "'");
