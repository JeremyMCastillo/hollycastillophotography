<?php
class ModelTltBlogTltTag extends Model {
	public function addTltTag($data) {
		$this->event->trigger('pre.admin.tlttag.add', $data);

		$this->db->query("INSERT INTO " . DB_PREFIX . "tlttag SET sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', show_in_category_list='" . (int)$data['category'] . "'");
		...
		if (isset($data['keyword'])) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "tltblog_url_alias SET query = 'tlttag_id=" . (int)$tlttag_id . "', keyword = '" . $this->db->escape($data['keyword']) . "'");
		}

		$this->cache->delete('tltblog');

		$this->event->trigger('post.admin.tlttag.add', $tlttag_id);

		return $tlttag_id;
	}

	public function editTltTag($tlttag_id, $data) {
		$this->event->trigger('pre.admin.tlttag.edit', $data);

		$this->db->query("UPDATE " . DB_PREFIX . "tlttag SET sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', show_in_category_list='" . (int)$data['category'] . "' WHERE tlttag_id = '" . (int)$tlttag_id . "'");
		...
		$this->event->trigger('post.admin.tlttag.edit', $tlttag_id);
	}
	...
	public function deleteTltTag($tlttag_id) {
		$this->event->trigger('pre.admin.tlttag.delete', $tlttag_id);
		...
		$this->event->trigger('post.admin.tlttag.delete', $tlttag_id);
	}

	public function getTltTag($tlttag_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT keyword FROM " . DB_PREFIX . "tltblog_url_alias WHERE query = 'tlttag_id=" . (int)$tlttag_id . "') AS keyword FROM " . DB_PREFIX . "tlttag t LEFT JOIN " . DB_PREFIX . "tlttag_description td ON (t.tlttag_id = td.tlttag_id) WHERE t.tlttag_id = '" . (int)$tlttag_id . "' AND td.language_id = '" . (int)$this->config->get('config_language_id') . "'");
