...
class ControllerTltBlogTltBlog extends Controller {
    ...
    public function index() {
        ...
        if ($tltblog_info) {
            ...
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/portfolio/tltblog.tpl')) {
            ...
            $blogPosts = $this->model_tltblog_tltblog->getTltBlogs();

            $data['posts'] = array();
            foreach($blogPosts as $post){
                $dateObj = strtotime($post['date_added']);
                
                $data['posts'][] = array(
                    "title" => $post["title"],
                    'image' => HTTPS_SERVER . 'image/' . $post['image'],
                    'description' => $post['description'],
                    'date_added' => date("F j Y"),
                    'meta_keyword' => $post['meta_keyword'],
                    'href' => $this->url->link('tltblog/tltblog', 'tltblog_id=' . $post['tltblog_id'])
                );
            }
            
            $data['tags'] = array();
            $blogTags = $this->model_tltblog_tltblog->getTltTags();
