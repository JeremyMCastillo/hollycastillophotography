<?php
class ControllerTltBlogTltBlogSeo extends Controller {
	public function index() {
		...
		if (isset($this->request->get['_route_'])) {
			...
			foreach ($parts as $part) {
				$query = $this->model_tltblog_url_alias->getUrlAliasByKeyword($this->db->escape($part));
	...
	public function rewrite($link) {
		...
		if (isset($url_info['query'])) {
			...
			foreach ($data as $key => $value) {
				if (isset($data['route'])) {
					if (($data['route'] == 'tltblog/tltblog' && $key == 'tltblog_id') || ($data['route'] == 'tltblog/tlttag' && $key == 'tlttag_id')){
